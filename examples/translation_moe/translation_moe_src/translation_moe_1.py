...
from .mean_pool_gating_network_1 import MeanPoolGatingNetwork, MeanPoolGatingNetwork1
...
class TranslationMoETask(TranslationTask):
    ...
    def build_model(self, cfg, from_checkpoint=False):
        ...
        model1 = models.build_model(cfg, self)
        if not self.uniform_prior and not hasattr(model1, "gating_network"):
            if self.cfg.mean_pool_gating_network:
                ...
                model1.gating_network = MeanPoolGatingNetwork1(
                    encoder_dim,
                    self.cfg.num_experts,
                    dropout,
                )
                model1.gating_network1 = MeanPoolGatingNetwork1(
                    encoder_dim,
                    self.cfg.num_experts,
                    dropout,
                )
    ...
    def _get_loss(self, sample, model, criterion):
        ...
        def get_lprob_yz(winners=None):
            ...
            if self.uniform_prior:
                ...
                model1 = model
                lprob_z1 = model1.gating_network1(encoder_out)
                if winners is not None:
                    lprob_z = lprob_z.gather(dim=1, index=winners.unsqueeze(-1))
                    lprob_z1 = lprob_z1.gather(dim=1, index=winners.unsqueeze(-1))
                lprob_yz = lprob_y + lprob_z.type_as(lprob_y)
                lprob_yz1 = lprob_y + lprob_z1.type_as(lprob_y)

            return lprob_yz, lprob_yz1

        with utils.model_eval(model):
            with torch.no_grad():
                lprob_yz, lprob_yz1 = get_lprob_yz()
                prob_z_xy = torch.nn.functional.softmax(lprob_yz, dim=1)
                prob_z_xy1 = torch.nn.functional.softmax(lprob_yz1, dim=1)
        assert not prob_z_xy.requires_grad or prob_z_xy1.requires_grad

        if self.hard_selection:
            winners = prob_z_xy.max(dim=1)[1]
            winners1 = prob_z_xy1.max(dim=1)[1]
            loss1, _ = get_lprob_yz(winners)
            _, loss2 = get_lprob_yz(winners1)
            loss = -loss1 - loss2
        else:
            lprob_yz, lprob_z1 = get_lprob_yz()
            loss = -LogSumExpMoE.apply(lprob_yz, prob_z_xy, 1) + (
                -LogSumExpMoE.apply(lprob_z1, prob_z_xy, 1)
            )
